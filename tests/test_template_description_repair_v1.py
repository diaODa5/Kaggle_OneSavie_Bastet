...
from src.generate_template_description_repair_v1 import (
    DESCRIPTION_REPAIRS,
    build_template_description_repair,
)


class TemplateDescriptionRepairV1Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_repairs_only_configured_descriptions(self) -> None:
        result, changed = build_template_description_repair(self.base)
        ...
        compare_cols = ["repo_path", "severity", "tag", "subtag"]
        pd.testing.assert_frame_equal(result[compare_cols], self.base[compare_cols])
    ...
    def test_repaired_descriptions_are_specific_not_templates(self) -> None:
        result, _ = build_template_description_repair(self.base)
