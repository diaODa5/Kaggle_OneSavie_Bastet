...
from src.generate_template_description_repair_v5 import (
    DESCRIPTION_REPAIRS,
    build_template_description_repair_v5,
)


class TemplateDescriptionRepairV5Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.base = pd.read_csv("outputs/submission_backup_3959_template_description_repair_v4.csv")

    def test_repairs_only_configured_descriptions(self) -> None:
        result, changed = build_template_description_repair_v5(self.base)
    ...
    def test_repaired_descriptions_are_specific_ascii_single_line(self) -> None:
        result, _ = build_template_description_repair_v5(self.base)
